...
<html>
	...
	<script type="text/javascript">
			...
			In.ready('ctcss','giscss','commonjs',function(){
				parent.exchangeDiv('','Trace',700);
			});
	</script>
	</head>
	<body>
		<input type="button" value="返回" onclick="parent.exchangeDiv('${ctx}/res/tracemgr/trace/trace_input.jsp','Trace',420);">
		<div style="float:right;width:auto;height:100%;overflow-x:hidden;overflow-y:auto;">
			 ...
			 <tbody>
			  <c:forEach items="${list}" var="item">
				  <tr>
				  	...
				  	<td><resinfo:resCommon keyValue="${item.qssbodm}" tableName="res_odm"/></td>
				    <td>${item.qsxxh}</td>
				    <td>${item.qssbdz}</td>
				  	<td>
				    ...
				    <td>
					    		<resinfo:resCommon keyValue="${item.zzsbodm}" tableName="res_odm"/>
				    </td>
				    <td>
				    			${item.zzxxh}
				    </td>
