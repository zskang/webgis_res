	...
	<script type="text/javascript">
			...
			In.ready('ctcss','giscss','dropdowncss','commonjs','qtipjs',function(){
				...
				$('.ui-div-list span:first-child a').each(function(){
					...
					var url = contextPath+'/res/publicmgr/jfxx!view.action?tipview=1&xtbh='+xtbh;
					$(this).qtip({
						hide:'unfocus',
						content:{
							text : 'Loading...',
				 ...
				 <span style="float:left;padding-left:5px">
					 <ul class="dropdown dropdown-horizontal">
						...
						操作
							...
							<ul  style="width:60px">
								<li><a href="javascript:void(0);" onclick="$.batchEdit('${ctx}/res/publicmgr/jfxx!toBatchEditPage.action')">批量编辑</a></li>
								<li><a href="javascript:void(0);" onclick="$.batchDelete('${ctx}/res/publicmgr/jfxx!batchDelete.action','AD701')">批量删除</a></li>
							</ul>
						</li>
					</ul>
				 </span>
				  <span style="float:left;padding-left:5px">
						<!-- <span class="dir">
						<baseinfo:expexcel cls="exprotButton" businessId="jfxxlist" name="导出excel"></baseinfo:expexcel>
						</span>
						 -->
				 </span>
					<span style="float:right;margin-right:10px">
					<a href="${ctx}/res/publicmgr/jfxx!query.action?${condition}" class="ui-gis-link"><<查询<<</a>
		  ...
		  <div>
				<c:forEach items="${map['rows']}" var="item">
					<div class="ui-div-list" style="width: 100%;">
						 ...
						 <a herf="javascript:void(0)" style="cursor:pointer" onclick="javascript:window.parent.openResourcetreediv('查看机房','${ctx}/res/publicmgr/jfxx.action?xtbh=${item.XTBH}')">${item.ZYMC }</a>
						 <a herf="javascript:void(0)" style="cursor:pointer;color:blue;" onclick="javascript:window.parent.openResourcetreediv('${item.ZYMC } - 连接管理','${ctx}/res/connectmgr/connect.action?xtbh=${item.XTBH }&zylx=AD701',1000,600)">连接管理</a>
						 </span>
						<ul>
							<li>
								产权性质：<baseinfo:dic displayProperty="LABLE" codevalue="${item.CQXZ }" columntype="property_right"></baseinfo:dic></li>
				        	<li>机房类型：<baseinfo:dic displayProperty="LABLE" codevalue="${item.JFLX }" columntype="JFLX"></baseinfo:dic></li>
				...
				<c:if test="${map['total'] > 0}">
					...
					<baseinfo:pagenation paginationCls="pagination_fixed" adjacents="1" records="${map['total']}" allowPageSelect="false"
						url="${ctx}/res/publicmgr/jfxx!list.action?${condition}"></baseinfo:pagenation>
				</c:if>
				<c:if test="${map['total'] <= 0}">
				  <p>&nbsp;</p>
				  <p>&nbsp;</p>     
