	...
	<script type="text/javascript">
			...
			In.ready('ctcss','giscss','dropdowncss','commonjs','qtipjs',function(){
				...
				$('.ui-div-list span:first-child a').each(function(){
					...
					var url = contextPath+'/res/publicmgr/zdxx!view.action?tipview=1&xtbh='+xtbh;
					$(this).qtip({
						hide:'unfocus',
						content:{
							text : 'Loading...',
	...
	<div class="ui-form-container">
			<div style="height:26px">
				<div class="ui-gis-fonter-bg">
				 ...
				 <span style="float:left;padding-left:5px">
					 <ul class="dropdown dropdown-horizontal">
						...
						操作
							...
							<ul  style="width:60px">
								<li><a href="javascript:void(0);" onclick="$.batchEdit('${ctx}/res/publicmgr/zdxx!toBatchEditPage.action')">批量编辑</a></li>
								<li><a href="javascript:void(0);" onclick="$.batchDelete('${ctx}/res/publicmgr/zdxx!batchDelete.action','A29')">批量删除</a></li>
							</ul>
						</li>
					</ul>
				 </span>
				 <span style="float:left;padding-left:5px">
						<!-- <span class="dir">
						<baseinfo:expexcel cls="exprotButton" businessId="zdxxlist" name="导出excel"></baseinfo:expexcel>
						</span>
						 -->
				 </span>
					<span style="float:right;margin-right:10px">
					<a href="${ctx}/res/publicmgr/zdxx!query.action?${condition}" class="ui-gis-link"><<查询<<</a>
		  ...
		  <div>
				<c:forEach items="${map['rows']}" var="item">
					<div class="ui-div-list" style="width: 100%;">
						 ...
						 <input type="checkbox" value="${item.XTBH }">
						 	<a herf="javascript:void(0)" style="cursor:pointer" onclick="javascript:window.parent.openResourcetreediv('查看站点','${ctx}/res/publicmgr/zdxx.action?xtbh=${item.XTBH}')">${item.ZYMC }</a>
						 	<button onclick="javascript:window.parent.locate('A29',${item.OBJECTID});">定位</button>
						 </span>
						<ul>
							<li>
								产权性质：
							...
							<li>
								站点类型：
								<baseinfo:dic displayProperty="LABLE" codevalue="${item.ZDLX }"
									columntype="ZDLX"></baseinfo:dic>
							</li>
							<li>
								所属县区：
				...
				<c:if test="${map['total'] > 0}">
					...
					<baseinfo:pagenation paginationCls="pagination_fixed" adjacents="1" records="${map['total']}" allowPageSelect="false"
						url="${ctx}/res/publicmgr/zdxx!list.action?${condition}"></baseinfo:pagenation>
