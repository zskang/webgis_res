...
<div class="ui-form-container">
	<div class="ui-title-banner-bg">
    	<div class="ui-title-banner-text">
    		巡检人搜索列表<font color="E4760A">(总计：${map['total']})</font>
    	</div>
	</div>
</div>
<div id="datepop">
	...
	<input class="button" type="button" value="确定" onclick="checkpath(false, 0, '')">		
</div>
...
<c:forEach items="${map['rows']}" var="item">
	<div class="ui-div-list">
		<ul>
			<li>姓名：<a href="javascript:void(0);" onclick="showinfowindow('${item.X}', '${item.Y}', '${item.LABEL}', 'man', '${item.PID}', '${item.BUSINESSTYPE}');">${item.LABEL}</a></li>
		...
		<ul>
			...
			<a href='javascript:void(0);' onclick="checkpath(true, 0, '${item.BUSINESSTYPE}')">今日轨迹</a>
			&nbsp;&nbsp;
			<a href='javascript:void(0);' onclick="show_datetime_div(event, '${item.PID}', '${item.BUSINESSTYPE}');">历史轨迹</a>
			</c:if>
			<c:if test="${realtime !='1' }">
				<a href='javascript:void(0);' onclick="show_datetime_div(event, '${item.PID}', '${item.BUSINESSTYPE}');">历史轨迹</a>
			</c:if>
