	...
	<script type="text/javascript">
			...
			In.ready('ctcss','giscss','dropdowncss','commonjs','qtipjs',function(){
				...
				$('.ui-div-list span:first-child a').each(function(){
					...
					var url = contextPath+'/res/groupcustomsmgr/broadband!view.action?tipview=1&xtbh='+xtbh;
					$(this).qtip({
						hide:'unfocus',
						content:{
							text : 'Loading...',
				 ...
				 <span style="float:left;padding-left:5px">
					 <ul class="dropdown dropdown-horizontal">
						...
						操作
							...
							<ul  style="width:60px">
								<li><a href="javascript:void(0);" onclick="$.batchDelete('${ctx}/res/groupcustomsmgr/broadband!batchDelete.action', 'A35')">批量删除</a></li>
							</ul>
						</li>
					</ul>
				 </span>
					<span style="float:right;margin-right:10px">
					<a href="${ctx}/res/groupcustomsmgr/broadband!query.action?${condition}" class="ui-gis-link"><<查询<<</a>
		  ...
		  <div>
				<c:forEach items="${map['rows']}" var="item">
					<div class="ui-div-list" style="width: 100%;">
						 ...
						 <input type="checkbox" value="${item.XTBH }">
						 	<a herf="javascript:void(0)" style="cursor:pointer" onclick="javascript:window.parent.openResourcetreediv('查看宽带小区','${ctx}/res/groupcustomsmgr/broadband.action?xtbh=${item.XTBH}')">${item.ZYMC }</a>
						 	<button onclick="javascript:window.parent.locate('A35',${item.OBJECTID});">定位</button>
						 </span>
						 <span class="dir">
							<a herf="javascript:void(0)" style="cursor:pointer;color:blue;display:none" onclick="javascript:window.location.href='${ctx}/res/groupcustomsmgr/broadband!input.action?xtbh=${item.XTBH}'">编辑</a>
						 </span>
						<ul>
							<li>
								宽带类型：<baseinfo:dic displayProperty="LABLE" codevalue="${item.BROADTYPE }" columntype="KDLX"></baseinfo:dic></li>
				        	<li>小区地址：${item.COMMADDR }</li>
				...
				<c:if test="${map['total'] > 0}">
					...
					<baseinfo:pagenation paginationCls="pagination_fixed" adjacents="1" records="${map['total']}" allowPageSelect="false"
						url="${ctx}/res/groupcustomsmgr/broadband!list.action?${condition}"></baseinfo:pagenation>
				</c:if>
				<c:if test="${map['total'] <= 0}">
				  <p>&nbsp;</p>
				  <p>&nbsp;</p>     
