...
<!--
	...
	In.ready('ztreejs','commonjs','resztree',function(){
		...
		loadTree('tree','${ctx}/res/basemgr/ys!getTreeNodes.action?xtbh='+$("#xtbh").val());
		$('#rooturl').val('${ctx}/res/basemgr/ys!getTreeNodes.action?xtbh='+$("#xtbh").val());//设置资源树根链接地址
		$('#contentFrame').get(0).src = "${ctx}/res/basemgr/ys!view.action?xtbh="+$("#xtbh").val();
	});
//-->
</script>
	<body>
