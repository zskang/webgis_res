...
<script type="text/javascript">
	In.ready('autocompleteJs','ztreejs','jggridlocalejs','wdatepickerjs','jqgridcss','commonjs','wresize',function() {
		...
		initPatrolGroupTree_003();
			//使用层布局
			var jqgrid = $("#btssitegrid").jqGrid({
				url : "${ctx}/res/publicmgr/btssite!listdata.action",
				datatype : "json",mtype : 'GET',rownumbers : true,
				colNames : [ '站点编号', '站点名称','网元名称', '中文名称','基站级别', '所属机房', '区域','入网时间', '代维公司', '维护组', '操作' ],
				colModel : [ 
					...
					{name : 'BTSSITE_ID',id : 'BTSSITE_ID',sortable : false,formatter : operateFmatter} 
					],
				rowNum : 15,autowidth : true,rowList : [ 15, 30, 50 ],pager : '#btssitepager',
				shrinkToFit : true,viewrecords : true,hidegrid : false,
				prmNames : {page : "pageNo",rows : "rows",sort : "sidx",order : "sord"},
				jsonReader : {root : "root",page : "pageNo",total : "totalPages",records : "total",repeatitems : false,id : "btssite_ID"}
	...
	function view(btssiteid) {
		window.location.href='${ctx}/res/publicmgr/btssite!view.action?btssiteid=' + btssiteid;
	}
	//查看
	function editit(btssiteid) {
		window.location.href='${ctx}/res/publicmgr/btssite!input.action?btssiteid=' + btssiteid;
	}
	//删除
	function deleteit(btssiteid) {
		if(confirm("确定要删除该信息吗？")){
		window.location.href = '${ctx}/res/publicmgr/btssite!delete.action?btssiteid='+ btssiteid;
		}
	}
	//查询
	function query() {
			...
			<table cellspacing="0" cellpadding="0" border="0" align="center">
				...
				<tr>
					...
					<td><baseinfo:dicselector name="bslevel" columntype="BS_LEVEL"
							type="select" cssClass="inputtext" isQuery="query"></baseinfo:dicselector>
					</td>
					<th>代维公司:</th>
				...
				<tr>
					<td colspan="4" align="center"><input type="button" value="查询"
						...
						onclick="add();" /> <baseinfo:expexcel cls="exprotButton"
							businessId="btssitelist" name="导出excel"></baseinfo:expexcel></td>
				</tr>
			</table>
		</div>
		<br />
