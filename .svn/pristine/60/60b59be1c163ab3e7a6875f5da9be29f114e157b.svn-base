	...
	<script type="text/javascript">
			...
			In.ready('ctcss','giscss','dropdowncss','commonjs','qtipjs',function(){
				...
				$('.ui-div-list span:first-child a').each(function(){
					...
					var url = contextPath+'/res/publicmgr/odf!view.action?tipview=1&xtbh='+xtbh;
					$(this).qtip({
						hide:'unfocus',
						content:{
							text : 'Loading...',
				 ...
				 <span style="float:left;padding-left:5px">
					 <ul class="dropdown dropdown-horizontal">
						...
						操作
							...
							<ul  style="width:60px">
								<li><a href="javascript:void(0);" onclick="$.batchEdit('${ctx}/res/publicmgr/odf!toBatchEditPage.action')">批量编辑</a></li>
								<li><a href="javascript:void(0);" onclick="$.batchDelete('${ctx}/res/publicmgr/odf!batchDelete.action','AA001')">批量删除</a></li>
							</ul>
						</li>
					</ul>
				 </span>
					<span style="float:right;margin-right:10px">
					<a href="${ctx}/res/publicmgr/odf!query.action?${condition}" class="ui-gis-link"><<查询<<</a>
		  ...
		  <div>
				<c:forEach items="${map['rows']}" var="item">
					<div class="ui-div-list" style="width: 100%;">
						 ...
						 <a herf="javascript:void(0)" style="cursor:pointer" onclick="javascript:window.parent.openResourcetreediv('查看odf','${ctx}/res/publicmgr/odf.action?xtbh=${item.XTBH}')">${item.ZYMC }</a>
						 </span>
						 <span class="dir">
							<a herf="javascript:void(0)" style="cursor:pointer;color:blue;" onclick="javascript:window.parent.openResourcetreediv('${item.ZYMC } - 连接管理','${ctx}/res/connectmgr/connect.action?xtbh=${item.XTBH }&zylx=AA001',1000,600)">连接管理</a>
						 </span>
						<ul>
							<li>
								产权性质：
