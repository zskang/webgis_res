	...
	<script type="text/javascript">
			...
			In.ready('ctcss','giscss','dropdowncss','commonjs','qtipjs',function(){
				...
				$('.ui-div-list span:first-child a').each(function(){
					...
					var url = contextPath+'/res/basemgr/gldxt!view.action?tipview=1&xtbh='+xtbh;
					$(this).qtip({
						hide:'unfocus',
						content:{
							text : 'Loading...',
				 ...
				 <span style="float:left;padding-left:5px">
					 <ul class="dropdown dropdown-horizontal">
						...
						操作
							...
							<ul  style="width:60px">
								<li><a href="javascript:void(0);" onclick="$.batchEdit('${ctx}/res/basemgr/gldxt!toBatchEditPage.action')">批量编辑</a></li>
								<li><a href="javascript:void(0);" onclick="$.batchDelete('${ctx}/res/basemgr/gldxt!batchDelete.action')">批量删除</a></li>
							</ul>
						</li>
					</ul>
				 </span>
					<span style="float:right;margin-right:10px">
					<a href="${ctx}/res/basemgr/gldxt!query.action?${condition}" class="ui-gis-link"><<查询<<</a>
		  ...
		  <div>
				<c:forEach items="${map['rows']}" var="item">
					<div class="ui-div-list" style="width: 100%;">
						 ...
						 <a herf="javascript:void(0)" style="cursor:pointer" onclick="javascript:window.parent.openResourcetreediv('查看杆路段系统','${ctx}/res/basemgr/gldxt.action?xtbh=${item.XTBH}',800)">${item.ZYMC }</a>
						 </span>
						 <span class="dir">
							<a herf="javascript:void(0)" style="cursor:pointer;color:blue;display:none" onclick="javascript:window.location.href='${ctx}/res/basemgr/gldxt!input.action?xtbh=${item.XTBH}'">编辑</a>
						 </span>
						<ul>
				...
				<c:if test="${map['total'] > 0}">
					...
					<baseinfo:pagenation paginationCls="pagination_fixed" adjacents="1" records="${map['total']}" allowPageSelect="false"
						url="${ctx}/res/basemgr/gldxt!list.action?${condition}"></baseinfo:pagenation>
				</c:if>
				<c:if test="${map['total'] <= 0}">
				  <p>&nbsp;</p>
				  <p>&nbsp;</p>     
