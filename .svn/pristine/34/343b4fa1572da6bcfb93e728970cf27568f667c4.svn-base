...
public abstract class BaseAction extends ActionSupport implements
	...
	// 信息层次常量
	// 错误信息层次
	public static final String INFO_ERROR_LEVEL = "error";
	// 警告信息层次
	public static final String INFO_WARNING_LEVEL = "warning";
	// 提示信息层次
	public static final String INFO_INFO_LEVEL = "info";
	// 成功信息层次
	public static final String INFO_SUCCESS_LEVEL = "success";
	 ...
	 *            列表
	 */
	public void converttojson(List<LinkedHashMap<String, Object>> list) {
		Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd").create();
		String json = gson.toJson(list);
		logger.info(json);
		outmessage(json);
	...
	public Map<String, Object> getDecodeCondtionMap(String condition,
		...
		if (StringUtils.isNotBlank(condition)) {// 直接进查询页面
			...
			for (int i = 0; i < conditions.length; i++) {
				...
				if (params.length > 1) {
					map.put(params[0],
							java.net.URLDecoder.decode(params[1], "utf-8") == null ? ""
									: java.net.URLDecoder.decode(params[1],
											"utf-8").trim());
				} else {
					map.put(params[0], "");
				}
		...
		if (map.containsKey("regionid")
				&& StringUtils.isNotBlank((map.get("regionid").toString()))) {
			regionids.addAll(baseInfoProvider.getAllRegionChildren(map.get(
					"regionid").toString()));
		} else {// 默认不选
		...
		if (user.isContractor()) {
			orgidinit = user.getOrgId();
		}
		if (StringUtils.isNotBlank((String) map.get("orgid"))) {
	...
	public Map<String, Object> getSerializeForm(String former) throws Exception {
		...
		if (StringUtils.isNotBlank(former)) {
			String[] decodeConditions = java.net.URLDecoder.decode(former,
					"utf-8").split("&");
			for (int i = 0; i < decodeConditions.length; i++) {
				String[] params = decodeConditions[i].split("=");
				if (params.length > 1) {
					map.put(params[0],
							params[1] == null ? "" : params[1].trim());
				} else {
					map.put(params[0], "");
				}
			}
			if (map.containsKey("xtbh")
					&& StringUtils.isNotBlank((map.get("xtbh").toString()))) {
