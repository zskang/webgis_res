	...
	<script type="text/javascript">
			...
			In.ready('ctcss','giscss','dropdowncss','commonjs','qtipjs',function(){
				...
				$('.ui-div-list span:first-child a').each(function(){
					...
					var url = contextPath+'/res/groupcustomsmgr/groupcustoms!view.action?tipview=1&xtbh='+xtbh;
					$(this).qtip({
						hide:'unfocus',
						content:{
							text : 'Loading...',
				 ...
				 <span style="float:left;padding-left:5px">
					 <ul class="dropdown dropdown-horizontal">
						...
						操作
							...
							<ul  style="width:60px">
								<li><a href="javascript:void(0);" onclick="$.batchDelete('${ctx}/res/groupcustomsmgr/groupcustoms!batchDelete.action', 'AA502')">批量删除</a></li>
							</ul>
						</li>
					</ul>
				 </span>
					<span style="float:right;margin-right:10px">
					<a href="${ctx}/res/groupcustomsmgr/groupcustoms!query.action?${condition}" class="ui-gis-link"><<查询<<</a>
		  ...
		  <div>
				<c:forEach items="${map['rows']}" var="item">
					<div class="ui-div-list" style="width: 100%;">
						 ...
						 <input type="checkbox" value="${item.XTBH }">
						 	<a herf="javascript:void(0)" style="cursor:pointer" onclick="javascript:window.parent.openResourcetreediv('查看集团客户','${ctx}/res/groupcustomsmgr/groupcustoms.action?xtbh=${item.XTBH}')">${item.GROUPNAME }</a>
						 	<button onclick="javascript:window.parent.locate('AA502',${item.OBJECTID});">定位</button>
						 </span>
						 <span class="dir">
							<a herf="javascript:void(0)" style="cursor:pointer;color:blue;display:none;" onclick="javascript:window.location.href='${ctx}/res/groupcustomsmgr/groupcustoms!input.action?xtbh=${item.XTBH}'">编辑</a>
						 </span>
						<ul>
							<li>
								集团客户级别：<baseinfo:dic displayProperty="LABLE" codevalue="${item.ZYJB }" columntype="JTKHJB"></baseinfo:dic></li>
				        	<li>客户类别：<baseinfo:dic displayProperty="LABLE" codevalue="${item.KHLB }" columntype="KHLB"></baseinfo:dic></li>
				        	<li>所属县区：<baseinfo:region displayProperty="regionname" id="${item.REGIONID }"></baseinfo:region></li>
				        	<li>联系人：${item.JKBLXR }</li>				        
				        </ul>
				        <ul>
				        	<li>经度：${item.LON}</li>
				        	<li>纬度：${item.LAT}</li>
				...
				<c:if test="${map['total'] > 0}">
					...
					<baseinfo:pagenation paginationCls="pagination_fixed" adjacents="1" records="${map['total']}" allowPageSelect="false"
						url="${ctx}/res/groupcustomsmgr/groupcustoms!list.action?${condition}"></baseinfo:pagenation>
				</c:if>
				<c:if test="${map['total'] <= 0}">
				  <p>&nbsp;</p>
				  <p>&nbsp;</p>     
