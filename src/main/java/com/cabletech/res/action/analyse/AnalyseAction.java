...
@Results( {
		@Result(name = "list", location = "/res/analyse/analyse_list.jsp") })
@Action("analyse")
public class AnalyseAction extends BaseAction {
	...
	public void initGridHeader(){
		...
		List<LinkedHashMap<String,Object>> gridHeader = analyseService.executeSql(header_sql);
	...
	public void initGridData(){
		...
		if(StringUtils.isNotBlank(start)){
			date_condition = " and r.createdate >= to_date('"+start+"','yyyy/mm/dd')";
		}
		if(StringUtils.isNotBlank(end)){
			date_condition = " and r.createdate <= to_date('"+end+"','yyyy/mm/dd')";
		}
		if(StringUtils.isNotBlank(start) && StringUtils.isNotBlank(end)){
			date_condition = " and r.createdate between to_date('"+start+"','yyyy/mm/dd') and to_date('"+end+"','yyyy/mm/dd')";
		}
		Map<String,Object> analyse = analyseService.getAnalyse(zylx, tjlx);
		...
		Map<String, Object> map = analyseService.queryPageMap("executeSql", params, super.getPage(), super.getLimit());
	...
	private String clob2String(Object obj){
		...
		CLOB clob = (CLOB)obj;
		try{
			result.append((clob == null || clob.length() == 0) ? null : clob.getSubString((long) 1, (int) clob.length()));
