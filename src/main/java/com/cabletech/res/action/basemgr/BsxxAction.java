...
import com.cabletech.res.service.basemgr.BsxxService;
...
@Results( {
		@Result(name = "frame", location = "/res/basemgr/bsxx/bsxx_frame.jsp"),
		@Result(name = "input", location = "/res/basemgr/bsxx/bsxx_input.jsp"),
		@Result(name = "view", location = "/res/basemgr/bsxx/bsxx_view.jsp"),
		@Result(name = "query", location = "/res/basemgr/bsxx/bsxx_query.jsp"),
		@Result(name = "list", location = "/res/basemgr/bsxx/bsxx_list.jsp"),
		@Result(name = "edit", location = "/res/basemgr/bsxx/bsxx_edit.jsp")})
@Action("bsxx")
public class BsxxAction extends BaseAction {

	private BsxxEntity entity;// 标石实体
	@Resource(name = "bsxxServiceImpl")
	private BsxxService bsxxservice;
	...
	public String list() throws Exception {
		...
		String method = "queryBsxxListNoSimid";
		if (conditionMap.containsKey("simid") && StringUtils.isNotBlank(conditionMap.get("simid").toString())) {
			method = "queryBsxxList";
		}
		map = bsxxservice.queryPageMap(method, conditionMap, super.getPage("page"), super.getLimit("rows"));		
	...
	public String save() {
		...
		if(bsxxservice.saveorupdate(entity)){
	...
	protected void prepareViewModel() throws Exception {
		...
		if (StringUtils.isNotBlank(xtbh)) {
			entity = bsxxservice.getbyid(xtbh);
			request.setAttribute("entity", entity);
		} else {
			entity = new BsxxEntity();
		}
	} 
	/**
	 * 获取资源关联树结点串
	...
	public String getTreeNodes()throws Exception{
		...
		out.print(bsxxservice.getTreeNodes(entity));
		out.flush();
		return null;
	}	
	...
	public String batchDelete(){
		...
		if(bsxxservice.batchDelete(xtbhs)){
			super.outmessage("删除成功！");
		}else{
			super.outmessage("删除失败！");
		}
